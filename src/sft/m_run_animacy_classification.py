...
def main():
    ...
    def label_old_format(token,label,targ_idx):
        old_label = []
        j = 0
        for i in range(len(token)):
            if i+1 in targ_idx:
                old_label.append(label[j])
                j = j+1
            else:
                old_label.append(-100)
        return old_label
    ...
    def tokenize_and_align_labels(examples):
        ...
        labels = []
        for i, label in enumerate(examples['anim_tags']):
            word_ids = tokenized_inputs.word_ids(batch_index=i)
            previous_word_idx = None
            label_ids = []
            #print("len lab",label)
            #print("word_id",word_ids)
            old_label = label_old_format(examples['tokens'][i], label,examples['target-indexes'][i])
            #print(label,examples['target-indexes'][i],old_label )
            for word_idx in word_ids:
                ...
                # We set the label for the first token of each word.
                    ...
                    label_ids.append(old_label[word_idx])
    ...
    def compute_metrics(p):
        ...
        score = precision_recall_fscore_support(flat_labels,flat_pred, average='macro')
